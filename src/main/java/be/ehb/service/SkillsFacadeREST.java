...
public class SkillsFacadeREST extends AbstractFacade<Skills> {
    ...
    @Path("PID/{id}")
    @Produces({MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON})
    public List<Skills> findByPID(@PathParam("id") Integer pid) {
        System.out.println("Searching skills for: " + pid);
        javax.persistence.Query q = getEntityManager().createNamedQuery("Skills.findByPID", Skills.class);
        q.setParameter("P_ID", pid);
